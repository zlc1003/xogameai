...
def train_model(model, num_epochs=10000):
    ...
    for epoch in tqdm.tqdm(range(num_epochs), desc="Training", position=0):
        ...
        while not st.isEnd:
            ...
            curr_board = torch.tensor(st.board).float().view(1, -1).cuda()  # Move tensor to GPU
        ...
        # Training the model
        states = torch.tensor(p1.states).float().view(-1, BOARD_ROWS * BOARD_COLS).cuda()  # Move tensor to GPU
        rewards = torch.tensor(p1.rewards).float().view(-1, 1).cuda()  # Move tensor to GPU
...
if __name__ == "__main__":
    model = TicTacToeNet().cuda()  # Move the model to GPU
